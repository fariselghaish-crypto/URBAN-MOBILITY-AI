...
DATA_DIR = Path("/Users/fariselghaish/Downloads/untitled folder")
DATA_FILE = DATA_DIR / "synthetic_realtime_urban_sensor_data.xlsx"
...
try:
    ...
    st.error(
        f"Could not find: {DATA_FILE}. Put synthetic_realtime_urban_sensor_data.xlsx in the same folder as this dashboard."
    )
    st.stop()
...
if mean_risk >= 70 or max_risk >= 90:
    system_status = "CRITICAL"
    status_class = "status-critical"
    status_text = "Immediate operational attention is required."
elif mean_risk >= 50 or mean_env >= 60:
    system_status = "ELEVATED"
    status_class = "status-elevated"
    status_text = "Risk is elevated. Active monitoring and local mitigation should be maintained."
else:
    system_status = "STABLE"
    status_class = "status-stable"
    status_text = "Current system conditions are stable under the selected thresholds."
...
<b>Alert interpretation:</b> Alert counts represent sustained or localised threshold exceedances. Therefore, a moderate
average system score can still coexist with alerts if short-duration spikes occur during specific times, modes or events.
</div>
""", unsafe_allow_html=True)
...
with c1:
    fig = px.bar(
        ...
        color="Mobility Mode",
        orientation="h",
        title="Decision Priority Score by Region and Mobility Mode",
        template="plotly_white",
        color_discrete_sequence=color_sequence,
        height=470,
        hover_data=["Mobility Mode", "Decision Priority Level", "Recommended Action"]
    )
    fig.update_layout(xaxis_title="Decision Priority Score (0-100)", yaxis_title="Region", legend_title="Mode")
    st.plotly_chart(fig, use_container_width=True)
